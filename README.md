...
- **8 built-in games** - Farkle, Pipzee, Cribbage, Euchre, Gin Rummy, Three Thirteen, Qwirkle, and Skyjo, each with its own scoring rules and win condition
...
- **Game-rule setup** - game-specific player limits and fixed endings replace generic controls where needed: Gin Rummy is 2 players, Qwirkle is 2-4, Euchre supports 2-6 variants, and Three Thirteen ends after 11 rounds. Entry Threshold appears only for games that use it
...
- **Turn haptics** - on compatible devices, the scorer gets a brief vibration when it becomes their turn; unsupported browsers simply use the visible turn notice
