...
function renderTable() {
  ...
  players.forEach((p, pi) => {
    ...
    dot.addEventListener('click', e => {
      e.stopPropagation();
      if (state.multiplayer) return; // color is assigned by join order in multiplayer rooms
...
document.getElementById('btn-save-turn').addEventListener('click', () => {
  ...
  if (state.multiplayer && state.mpScoringMode === 'each') {
    const value = clampScore(parseInt(inputs[0] ? inputs[0].value : '0'));
    mpSend({ type: 'submit-score', value });
    closeTurnModal();
    return;
  }

  if (state.multiplayer && state.mpScoringMode === 'host') {
    const values = Array.from(inputs).map(inp => clampScore(parseInt(inp.value)));
  ...
  const scores = Array.from(inputs).map((inp, pi) => {
    const raw = clampScore(parseInt(inp.value));
...
function mpHandleMessage(msg) {
  switch (msg.type) {
    ...
    case 'roster-update': mpApplyRoster(msg.players); break;
...
function mpOnJoined(msg) {
  ...
  mpApplyRounds(msg.rounds, msg.roundSubmitted || msg.players.map(() => false));
...
function mpApplyRounds(rounds, roundSubmitted) {
  state.rounds = rounds;
...
function mpOnError(msg) {
  if (msg.code === 'room-full') {
    if (!document.getElementById('modal-player-name').classList.contains('hidden')) {
      mpShowPlayerNameError('This room already has 8 players.');
    } else {
...
function mpOpenPlayerOptions(pi) {
  const p = state.players[pi];
  if (!p || p.id === state.mpPlayerId) return; // host can't remove themselves this way
